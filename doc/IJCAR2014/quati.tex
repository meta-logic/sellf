...
$SubSig$ ::= & 
\multicolumn{5}{l}{\texttt{subexp} $\tup{String}$ $\tup{SubType}$ .  $\mid$  \texttt{subexpctx} $\tup{String}$ $\tup{CtxType}$ $\tup{Side}$ .}
\\[2pt]
$Bipoles$ ::= & {\texttt{not} $\tup{Atom}$ \texttt{*} $\tup{Body}$} & \quad $Atoms$ ::= & $\tup{Side}$ $\tup{Terms}$\\[2pt]
$Body$ ::= & 
\multicolumn{5}{l}{\texttt{top} $\mid$ \texttt{one} $\mid$ $\tup{Atom}$ $\mid$ $\tup{Body} | \tup{Body}$ $\mid$ $\tup{Body} \& \tup{Body}$ }
\end{tabular}
