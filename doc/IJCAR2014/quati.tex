...
$BodyPos$ ::= & \texttt{one} $\mid$ $BodyNeg$ $\mid$ \texttt{[}$\tup{String}$\texttt{]!} $BodyNeg$ $\mid$ $BodyPos$ \texttt{*} $BodyPos$ $\mid$ \\[2pt]
$BodyNeg$ ::= & 
{\texttt{top}  $\tup{MarkAtoms}$ $\mid$ $\tup{BodyNeg} | \tup{BodyNeg}$ $\mid$ $\tup{BodyNeg} \& \tup{BodyNeg}$ }
\end{tabular}
...
A Quati program is a SELL theory with some more annotations. The syntax of Quati programs is given in Figure~\cite{fig:syntax}. 
