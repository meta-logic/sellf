...
$BodyNeg$ ::= & 
{\texttt{top} $\mid$ \texttt{bot} $\mid$ $\tup{MarkAtoms}$ $\mid$ $\tup{BodyNeg} | \tup{BodyNeg}$ $\mid$ $\tup{BodyNeg} \texttt{\&} \tup{BodyNeg}$ }
\end{tabular}
...
Quati's underlying logic, Linear Logic with Subexponentials (SELL)~\cite{nigam09ppdp}, is a powerful framework for specification of proof systems. 
...
A Quati program consists of three parts: (1) the term and kind declarations, specifying hte constants in the $Term$ set; (2) the 
subexponential signature declaration, $SubSig$, specifying the subexponentials 
used and their relation, $\preceq$; and (3) a set of $Bipoles$, specifying the proof rules of the proof system. 

\paragraph{Term and Kind Declaration}
\todo{To be completed}
\begin{verbatim}
%%%%%%%%%%%%%%%%%%% Term Signature %%%%%%%%%%%%%%%%%%%%
...
\paragraph{Subexponential Signature} The following set of subexponential signatures is used for specifying the proof system MLJ:
...
\paragraph{Bipoles}
The second part of the Quati is composed by $Bipoles$. The concrete syntax for SELL connectives
is depicted in Figure~\ref{fig:syntax_ll}. The class of $Bipoles$ formulas often appear in Proof Theory
Literature due to its good Focusing behaviour~\cite{andreoli}. The following Bipoles specify, 
...
\end{verbatim}
The head of these bipoles, the formula \texttt{(not (lft (imp A B)))} and
...
\paragraph{Feature 1: Proof Rule Construction}
By using the following command: \todo{Add command}

Quati constructs the corresponding proof rule of a bipole as described in \cite{nigam14ebl}. It 
