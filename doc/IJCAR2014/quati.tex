...
% For example, the cut-elimination result for
...
% permutation lemmas was not true. 
...
In \cite{nigam13iclp}, we showed how to reduce the problem of proving permutation lemmas to an Answer-Set
Program~\cite{gelfond}. That is given a proof system, $\Pscr$, satisfying some properties, we reduced the problem 
of checking whether a rule $r_1$ in $\Pscr$ always permutes over $r_2$ in $\Pscr$ to an Answer-Set Program.
