...
let rec hnorm term =
  match Term.observe term with
    ...
    | Term.PRED(str, t1, p) -> Term.PRED(str, hnorm t1, p)
    | Term.TENSOR (t1, t2) -> Term.TENSOR (hnorm t1, hnorm t2)
    | Term.ADDOR (t1, t2) -> Term.ADDOR (hnorm t1, hnorm t2)
    | Term.PARR (t1, t2) -> Term.PARR (hnorm t1, hnorm t2)
    ...
    | Term.QST (str, t1) -> Term.QST (hnorm str, hnorm t1)
    | Term.WITH (t1, t2) -> Term.WITH (hnorm t1, hnorm t2)
    | Term.NOT (t1) -> Term.NOT (hnorm t1)
    ...
    | Term.EXISTS(str,n,t) -> Term.EXISTS(str,n,hnorm t)
    ...
    | Term.SUSP(t,ol,nl,e) ->
        let t = hnorm t in
          begin match Term.observe t with
            ...
            | Term.QST (str, t1) -> Term.QST(hnorm (Term.susp str ol nl e), hnorm (Term.susp t1 ol nl e))
...
let rec deep_norm t =
  let t = hnorm t in
    match Term.observe t with
      ...
      | Term.PRED (str,t1, p) -> Term.PRED (str, deep_norm t1, p)
      ...
      | Term.ADDOR (t1,t2) -> Term.ADDOR (deep_norm t1, deep_norm t2)
      | Term.PARR (t1,t2) -> Term.PARR (deep_norm t1, deep_norm t2)
