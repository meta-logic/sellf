(* 
 * Generates derivations for a bipole.
 * This derivation has exactly one positive phase followed by one negative
 * phase.
 *
...
(* This method is supposed to return a list of macro rules *)

(* Positive phase *)
let rec posDer seq const form = 

  (* RULE: + *)
  | ADDOR (f1, f2) -> 
    (posDer seq const f1) :: (posDer seq const f2)
...
(* Negative Phase *)
let rec negDer seq const formlst = 
