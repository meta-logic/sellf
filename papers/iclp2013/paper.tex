...
to the right.
% while some linear logic presentations~\cite{andreoli92jlc} 
% have sequents of the form $\vdash \Theta \mid \Delta$ where $n = 0$ and $m = 2$. Other examples 
% can be found in \cite{nigam11lsfa}.
% \footnote{Notice that the sequents with contexts are not 
% hypersequents~\cite{avron91amai}.} 
...
% The formula that is being introduced in the conclusion is 
% called principal formula, while its subformulas that appear in the premises are called
% active formulas.
% We classify a context in a premise or in the conclusion of an inference rule 
% as a \emph{side-context} if it does not 
% contain the principal formula being introduced nor any of the active formulas, 
% and we classify it as an \emph{active context} otherwise. 
%For instance, the inference rules for $\tensor_R$ of linear logic and $\land_R$ of 
%intuitionistic logic are shown below:
...
% The contexts in the premises and the conclusion sequent to the left-hand-side
% are side-contexts, namely the contexts marked with an underline, 
% while the contexts in the premises and in the conclusion to the right-hand-side
% are active-contexts. 
...
% Notice that the context appearing on the right-hand-side of sequents, containing
% exactly one formula\footnote{Some presentations of intuitionistic
% systems allow no formula to appear on the right-hand-side, that is, these contexts
% have at most one formula. As the systems are equivalent, we do not make this difference.}
% is a
% 
% An \emph{exactly one} context, as the name indicates, contains always exactly
% one formula, as it is the case for the context 
% on the right side of LJ sequents.
% G: Exactly one or at most one?? This can be empty, no?

%In particular, the rules 
%in linear logic specify that the formulas in the contexts to the left and right
%are all instantiated by \emph{multisets} of formulas, that is, they are split among the 
%premises. This is observed by the fact that formulas are not allowed to contract nor
%weaken.
%Thus the formulas, $\Gamma, \Gamma', \Delta, \Delta'$, appearing in the conclusion of rule $\tensor_R$ 
%are split among the premises. We call such premises as \emph{bounded}. The rules in LJ, on the other hand, specify that 
%the context to the left-hand-side of the sequent behaves as a set, as formulas in this 
%context can be weakened and contracted. Thus, the formulas, $\Gamma$, appearing in the conclusion 
%of instances of the $\land_R$ rule are copied among its premises and not split. We call such contexts
%as \emph{unbounded}. Moreover, LJ sequents also specifies that there is exactly one formula
%to their right-hand-side of LJ sequents. We classify this context as \emph{exaclty one}. 
% This classification of contexts guide the shape 
% of derivations. For instance, in the $\land_R$ rule above, the context appearing
% to the left is unbounded, thus the formulas $\Gamma$ are copied among the contexts.
% 
% the left-introduction rule for $\iimp$ has the following shape:
% \[
% \infer[\iimp_L]{\Gamma, A\iimp B \vdash C}{
%   \Gamma, A\iimp B \vdash  A 
%   & 
%   \Gamma, A\iimp B, B \vdash C}
% \]
% Note that if the formula $C$ appeared in the left-premise as it appears in the
% right-premise, the sequent would have two formulas on the right. This conflicts
% with LJ's specification that sequents can only have one formula on the right
% side of the sequent. On the other hand, the formulas, $\Gamma$, appearing in 
% the unbounded context to the left of the turnstyle are copied among the premises. 
%
% On the other hand, the bounded contexts to the left and to the right of the rule
% $\tensor_R$ shown above are split among the premises. No formula is erased or copied.
...
% G: removing this condition. We are not sure why is it that we need it anymore.
%Moreover, assume that 
%any inference rule $R$ has the property:
%\begin{MyQuote}
%Any context variable $\Gamma_i$ appearing in a premise also appears in its conclusion.
%\label{qu:inference}
%\end{MyQuote}
%That is, an inference rule does not create a new context variable. For instance, the rule 
%below,
%\[
% \infer{\Gamma_1 \vdash \Gamma_2, \star A}{\Gamma_1 \vdash \Gamma_3, A},
%\]
%is not allowed because the context variable $\Gamma_3$ does not appear in its conclusion.
%This does not mean, however, that new formulas do not appear in the premise, such as in 
%the cut-rule.
...
%\[
% \begin{array}{l@{\quad}r@{\quad}l@{\qquad}@{\qquad}r@{\quad}l@{\qquad}r@{\quad}l}
%\toprule
%{\textrm{Phase 1:}} 
...
\newblock {\em Inf. \& Comp.\/}~{\em 179,\/}~1 (Nov.), 19--75.
...
\newblock {\em Theor. Comput. Sci.\/}~{\em 135}.
...
\newblock In {\em {The Collected Papers of Gerhard Gentzen}}, {M.~E. Szabo}, 68--131.
...
\bibitem[\protect\citeauthoryear{Lutovac and Harland}{Lutovac and
  Harland}{}]{lutovac13comsis}
{\sc Lutovac, T.} {\sc and} {\sc Harland, J.}
\newblock A contribution to automated-oriented reasoning about permutability of
  sequent calculi rules.
\newblock {\em Computer Science and Information Systems\/}~{\em 10}.
...
\newblock From proofs to focused proofs: a modular proof of focalization in
  linear logic.
\newblock In {\em CSL 2007} Springer, 405--419.
