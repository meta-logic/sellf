...
\emph{proof tree schemas}, defined in the module \texttt{ProofTreeSchema} and
using the modules \texttt{SequentSchema} and \texttt{ContextSchema}. As the name
...
non-empty sets of formulas. The generic contexts are implemented as pairs
\texttt{string * int}, where the first element is the name of the context (or
subexponential) and the second is a unique identifier. The constraints that will
...
Quati was tested using the following proof systems: LK, LJ, MLJ, LL, S4, G1m. On
most cases, each permutation lemma can be checked in less than one second. The
implementation can be downloaded at \url{\textbf{LINK!!!}}.
% TODO: add the web interface address????
